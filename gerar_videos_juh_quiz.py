...
def draw_text_in_box(draw, texto, box, max_size=58, min_size=22, fill=BLACK, bold=True, align="center", max_lines=None):
    x1, y1, x2, y2 = box
    bw = x2 - x1
    bh = y2 - y1
...
def fundo_base():
    if not FUNDO.exists():
        raise FileNotFoundError(f"Fundo não encontrado: {FUNDO}. Envie assets/fundo_juh_quiz.png ao GitHub.")
    img = Image.open(FUNDO).convert("RGB")
    resampling = getattr(Image, "Resampling", Image)
    return ImageOps.fit(img, (W, H), method=resampling.LANCZOS, centering=(0.5, 0.5))
...
def validar_pergunta(pergunta):
    ...
    if pergunta.get("usar_imagens", False):
        ...
        if len(imagens) != 3:
            raise ValueError(f'A pergunta "{pergunta["pergunta"]}" está com usar_imagens=True e precisa ter exatamente 3 imagens.')
        for caminho in imagens:
            if not Path(caminho).exists():
                raise FileNotFoundError(f'Imagem ausente na pergunta "{pergunta["pergunta"]}": {caminho}')

tema_atual_global = ""

def desenhar_tema(draw, tema):
    box = [250, 335, 780, 405]
    draw.rounded_rectangle([box[0] + 4, box[1] + 5, box[2] + 4, box[3] + 5], radius=32, fill=(215, 166, 28))
    draw.rounded_rectangle(box, radius=32, fill=YELLOW)
    draw_text_in_box(draw, tema.upper(), [box[0] + 18, box[1] + 4, box[2] - 18, box[3] - 4], max_size=28, min_size=16, fill=DARK_BLUE, bold=True, max_lines=1)

def desenhar_badge_pergunta(draw, numero, total):
    box = [820, 335, 1010, 405]
    draw.rounded_rectangle([box[0] + 4, box[1] + 5, box[2] + 4, box[3] + 5], radius=32, fill=(215, 166, 28))
    draw.rounded_rectangle(box, radius=32, fill=YELLOW)
    draw_text_in_box(draw, f"{numero}/{total}", [box[0], box[1], box[2], box[3]], max_size=30, min_size=22, fill=DARK_BLUE, bold=True)

def desenhar_sombra_card(img, box, radius=56, opacity=30, blur=10, dx=7, dy=9):
    sombra = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    sd = ImageDraw.Draw(sombra)
    sd.rounded_rectangle([box[0] + dx, box[1] + dy, box[2] + dx, box[3] + dy], radius=radius, fill=(0, 0, 0, opacity))
...
def desenhar_card_pergunta_sem_imagens(img, pergunta, numero, total, estado, timer):
    ...
    draw_text_in_box(draw, f'{numero}. {pergunta["pergunta"]}', [82, 505, W - 82, 860], max_size=66, min_size=34, fill=BLACK, bold=True, max_lines=5)
    timer_y = 1015
    alt_top = 1180
...
def desenhar_card_pergunta_com_imagens(img, pergunta, numero, total, estado, timer):
    ...
    draw = ImageDraw.Draw(img)
    draw.rounded_rectangle(card, radius=58, fill=WHITE)
    draw_text_in_box(draw, f'{numero}. {pergunta["pergunta"]}', [70, 470, W - 70, 660], max_size=58, min_size=28, fill=BLACK, bold=True, max_lines=4)
    ...
    for i in range(3):
        ...
        if rotulos[i]:
            label_y = y0 + box_h - 58
            draw.rounded_rectangle([x + 14, label_y, x + box_w - 14, y0 + box_h - 12], radius=18, fill=(35, 35, 42))
            draw_text_in_box(draw, rotulos[i], [x + 22, label_y + 2, x + box_w - 22, y0 + box_h - 13], max_size=24, min_size=15, fill=WHITE, bold=True)
    timer_y = 1175
    alt_top = 1335
    return img, timer_y, alt_top
...
def desenhar_timer(draw, estado, timer, timer_y):
    ...
    draw_text_in_box(draw, txt, [x0, timer_y, x1, timer_y + d], max_size=48, min_size=30, fill=WHITE, bold=True)
...
def desenhar_alternativas(draw, pergunta, estado, alt_top):
    ...
    for i, alt in enumerate(alternativas):
        ...
        if estado == "answer" and correta:
            ...
            shadow_fill = (205, 210, 220)

        draw.rounded_rectangle([x + 4, y + 7, x + w + 4, y + opt_h + 7], radius=999, fill=shadow_fill)
        draw.rounded_rectangle([x, y, x + w, y + opt_h], radius=999, fill=fill)

        d = 112
        cx = x - 48
        draw.ellipse([cx + 3, y + 5, cx + d + 3, y + d + 5], fill=(180, 180, 190))
        draw.ellipse([cx, y, cx + d, y + d], fill=circle_fill,
                     outline=(255, 210, 235) if not (estado == "answer" and correta) else (220, 255, 210), width=4)

        letra = "✓" if estado == "answer" and correta else letras[i]
        draw_text_in_box(draw, letra, [cx, y, cx + d, y + d], max_size=54, min_size=30, fill=(255, 255, 255), bold=True)
        draw_text_in_box(draw, alt, [x + 92, y + 10, x + w - 35, y + opt_h - 10], max_size=44, min_size=22, fill=text_fill, bold=True, align="left", max_lines=2)
...
def render_frame(tema, pergunta, numero, total, estado, timer=None):
    ...
    img = fundo_base()
    if pergunta.get("usar_imagens", False):
        img, timer_y, alt_top = desenhar_card_pergunta_com_imagens(img, pergunta, numero, total, estado, timer)
    else:
        img, timer_y, alt_top = desenhar_card_pergunta_sem_imagens(img, pergunta, numero, total, estado, timer)
    draw = ImageDraw.Draw(img)
...
def tts_salvar(texto, caminho):
    ...
    if caminho.exists():
        caminho.unlink()
    executar(["edge-tts", "--voice", VOZ, "--rate", VELOCIDADE_VOZ, "--text", limpar_tts(texto), "--write-media", str(caminho)])
...
def duracao_audio(caminho):
    cmd = ["ffprobe", "-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", str(caminho)]
    return float(subprocess.check_output(cmd, text=True).strip())

def criar_beep(caminho, frequencia=950):
    executar(["ffmpeg", "-y", "-f", "lavfi", "-i", f"sine=frequency={frequencia}:duration=0.14", "-af", "volume=0.55,apad=pad_dur=1", "-t", "1.0", "-c:a", "aac", "-b:a", "160k", "-ar", str(AUDIO_HZ), "-ac", str(AUDIO_CHANNELS), str(caminho)])

def criar_clipe_imagem(img_path, duracao, saida, audio=None):
    duracao = float(duracao)
    if audio:
        cmd = ["ffmpeg", "-y", "-loop", "1", "-i", str(img_path), "-i", str(audio), "-t", f"{duracao:.3f}", "-vf", f"scale={W}:{H},fps={FPS},format=yuv420p", "-af", f"aresample={AUDIO_HZ}:async=1:first_pts=0,apad", "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "160k", "-ar", str(AUDIO_HZ), "-ac", str(AUDIO_CHANNELS), "-shortest", "-movflags", "+faststart", str(saida)]
    else:
        cmd = ["ffmpeg", "-y", "-loop", "1", "-i", str(img_path), "-f", "lavfi", "-i", f"anullsrc=channel_layout=stereo:sample_rate={AUDIO_HZ}", "-t", f"{duracao:.3f}", "-vf", f"scale={W}:{H},fps={FPS},format=yuv420p", "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "160k", "-ar", str(AUDIO_HZ), "-ac", str(AUDIO_CHANNELS), "-shortest", "-movflags", "+faststart", str(saida)]
    executar(cmd)

def concatenar_clipes(clipes, saida):
    lista = PASTA_TMP / "concat.txt"
    lista.write_text("\n".join(f"file '{Path(c).resolve().as_posix()}'" for c in clipes), encoding="utf-8")
    executar(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(lista), "-c", "copy", "-movflags", "+faststart", str(saida)])
...
def preview():
    ...
    for p in perguntas:
        validar_pergunta(p)
        img = render_frame(tema, p, numero=perguntas.index(p) + 1, total=len(perguntas), estado="reading")
